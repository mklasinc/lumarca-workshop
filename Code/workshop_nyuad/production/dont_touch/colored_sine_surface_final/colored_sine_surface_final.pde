...
static public void main(String args[]) {
  PApplet.main(new String[] { "--present", "colored_sine_surface_final" });
}
...
void color_change(){
  int new_time_s = second();
  int elapse_time = new_time_s - old_time_s;
  
  if(abs(elapse_time) > color_change_interval-1){
  ...
  if(hue_change_underway){
    ...
    //println("change color!");
     ...
     if(hue_change_val > hue_change_limit){
       hue_change_val = 0;
       hue_change_underway = false;
     }
    
  };
  
  
  //println(old_time_s); // Values from 0 - 59
  /*if(hue < 0){
    hue_increase *= -1;
  }else if(hue > 360){
    hue_increase *= -1;
  ...
  globe_color = color(hue,100,100);
  bottom_color = color(360-hue,100,100);*/
}
...
void sineSurface() {
  ...
  for(int i=0; i<num_of_strings; i+=1) {
    ...
    //PARTICLES COLOR
    fill(white);                                                                    // draw a rectangle at that intersect
    ...
    //fill(0, 0, water_color); //BOTTOM COLOR
    fill(bottom_color);
  ...
  for(int i = 0; i < num_of_strings; i += 1) {
    if((x_by_ind[i] >= (x - rad)) && (x_by_ind[i] <= (x + rad))) {                  // if a num_of_strings's x coord is close enough to the globe's center
      ...
      if(local_hyp <= rad) {                                                        // if the num_of_strings's xz coord is close enough to the globe's center
        ...
        fill(white);                                                                  // Fill the surface pixels White
