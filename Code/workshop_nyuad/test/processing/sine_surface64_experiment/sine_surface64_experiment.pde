...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Key interaction global vars
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int vj_counter = 0;
boolean SKETCH_TWO = false;
color bottom_color = color(0,0,0); 
float hue = 0.1;
...
static public void main(String args[]) {
  PApplet.main(new String[] { "--present", "sine_surface64_experiment" });
}
...
void colorChange(){
  ...
  hue += hue_increase;
  bottom_color = color(hue,100,100);
...
void sineSurface() {
  ...
  float[] amp = new float[trail];                               //  amplitude of wave @ num_of_strings point according to mouse movement & beatmapping
  ...
  for(int i=0; i<num_of_strings; i+=1) {
    ...
    for(int x = 0; x < trail; x ++ ) {
      ...
      float add_to_amp = map(mouseY, 0, height, -0.01, 0.05);
      
      if(SKETCH_TWO){
        amp[x] += add_to_amp;
      }
       /////////////////////HEEEEEERREREREREREREREREREER
      //amp[x] += map(mouseY, 0, height, 0, 0.1) + (noise(mouseY*0.001, x)-0.5)*100.0 * sin((2 * PI * distance_since_pass_fraction[x]));
      //amp[x] *= 0.01;
      //println(distance_since_pass_fraction[x]);
    ...
    // compensate for projection morphing IN INCHES
    float y_bot_proj = y_bot_coord * depth / z_by_ind[i];
    float y_height_proj = y_top_proj - y_bot_proj;
    fill(0,0,100);                                                                    // draw a rectangle at that intersect

    //THESE ARE TOP SLIVERS  
  ...
  for(int i = 0; i < num_of_strings; i += 1) {
    if((x_by_ind[i] >= (x - rad)) && (x_by_ind[i] <= (x + rad))) {                  // if a num_of_strings's x coord is close enough to the globe's center
      ...
      if(local_hyp <= rad) {                                                        // if the num_of_strings's xz coord is close enough to the globe's center
        ...
        /* Top dot
        ---------------------------------------------------------*/
        fill(0,100,100);                                   // Fill the globe pixels this color
        float left1           = i * (width) / num_of_strings;
        ...
        fill(0,0,100);                                                                  // Fill the surface pixels White
